...
export default function PagosPage() {
  ...
  const concepts = [
    { id: 'Mensualidad', title: 'Mensualidad', price: 30, description: 'Pago correspondiente al mes en curso.' },
    { id: 'Tryout', title: 'Tryout / Scouting', price: 15, description: 'Pago de evaluación de habilidades.' },
    { id: 'Clinica', title: 'Clínica Deportiva', price: 40, description: 'Sesión intensiva de entrenamiento.' },
    { id: 'Amistoso', title: 'Juego Amistoso', price: 5, description: 'Cuota para arbitraje y campo.' },
  ]
      ...
      {!selectedConcept ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {concepts.map((concept) => (
            <button
              key={concept.id}
              onClick={() => handleSelect(concept.id, concept.price)}
            ...
            >
              <div className="flex justify-between items-start mb-2">
                <h3 className="font-bold text-gray-900 text-lg group-hover:text-kasa-vinotinto transition-colors">{concept.title}</h3>
                <span className="font-bold text-kasa-vinotinto bg-red-50 px-3 py-1 rounded-full">${concept.price}</span>
              </div>
          ...
          <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200 opacity-60 cursor-not-allowed">
            ...
            <p className="text-sm text-gray-500">Próximamente disponible en tienda.</p>
          </div>
        </div>
      ) : (
        <div className="bg-white p-6 sm:p-8 rounded-2xl border border-gray-200 shadow-sm max-w-2xl mx-auto">
          <div className="flex justify-between items-center mb-6 pb-6 border-b border-gray-100">
            <div>
              ...
              <h3 className="font-bold text-xl text-gray-900">{selectedConcept}</h3>
            </div>
            <div className="text-right">
              <p className="text-sm text-gray-500">Total a pagar:</p>
              <span className="font-bold text-2xl text-kasa-vinotinto">${amount}</span>
            </div>
          </div>
